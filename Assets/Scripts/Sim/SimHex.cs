    ...
    */
	...
	public InputStatus CheckInputs()
	{
		...
		foreach (ResRequired rr in type.resourcesRequired)
		{
			...
			{ //drawing from local- self and neighbors 
	...
	public void ConsumeInputs()
	{
		...
		foreach (ResRequired rr in type.resourcesRequired)
		{
			...
			if (rr.isConsumed)
			{
				...
				if (!rr.local)
				{ //consumes from global pool

					GlobalPool.Consume(rr.id, rr.amount);

				}
				else if (rr.radius > 0)
				{ //consumes from self and neighbors
	...
	public void CreateOutputs(int tickNum)
	{
		...
		foreach (ResProduced rp in type.resourcesProduced)
		{
			...
			else
			{
				...
				if (!rp.local)
				{ //producing to global pool 

					if (rp.cap > 0)
					{ //don't produce over cap 
				...
				{ //producing to self and neighbors 
